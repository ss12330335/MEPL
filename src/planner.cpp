...
Planner::Planner(const char *domain) {
	...
	printf("\nPreprocessing Start!\n...\n");
	parser.exec(domain, true);
	printf("\nProcessing Completed!\n");

	// print compile result
	//cout << "epis_actions.size() = " << sActions.size() << endl;
	//cout << "ontic_actions.size() = " << oActions.size() << endl;
	//cout << endl << endl;
	//init.print();
	//cout << endl << endl;
	//goal.print();
...
void Planner::exec() {
	printf("\nPlanning Start!\n...\n");
	if (init.negEntail(goal, constraint)) {
	...
	LAONode initNode(init, 0, false, false);
...
int Planner::insertNode(KLDNF& kb) {
	int nodeSize = allNodes.size(), i = 0;
	bool isGoal = kb.negEntail(goal, constraint);
	if(isGoal) {
		if(goalPos == -1) {
			goalPos = nodeSize;
	...
	for(;i < nodeSize;i++) {
		...
		if(kb.strongEquivlent(allNodes[i].kb, constraint)) {
			return i;
		}
	}
	allNodes.push_back(LAONode(kb, 
		heuristicHelper.getHeuristicValue(kb), false, false));
	return nodeSize;
}

bool Planner::hasExplored(KLDNF& kb) {
	int nodeSize = allNodes.size(), i = 0;
	for(;i < nodeSize;i++) {
		if(kb.strongEquivlent(allNodes[i].kb, constraint)) {
			break;
...
void Planner::exploreNode(int index) {
	...
	int onticSize = oActions.size(), sensSize = sActions.size();
	allEdges[index].clear();
	//cout << "======================================" << endl;
	for(int i = 0;i < onticSize;i++) {
		if(curKB.negEntail(oActions[i].preCon, constraint)) {
			KLDNF resKB = curKB;
			vector<KLDNF> satEff;
			for(auto ite = oActions[i].eff.begin();ite != oActions[i].eff.end();ite++) {
				if(curKB.negEntail(ite->condition, constraint)) {
					satEff.push_back(ite->effect);
				}
			}
			for(auto ite = satEff.begin();ite != satEff.end();ite++) {
				resKB = resKB.update(*ite, constraint).minimal(constraint);
			}
	...
	for(int i = 0;i < sensSize;i++) {
		if(curKB.negEntail(sActions[i].preCon, constraint)) {
			KLDNF posKB = curKB.revision(sActions[i].posRes, constraint).minimal(constraint);
			KLDNF negKB = curKB.revision(sActions[i].negRes, constraint).minimal(constraint);
			vector<int> temp;
			temp.push_back(insertNode(posKB));
			temp.push_back(insertNode(negKB));
			allEdges[index].push_back(LAOEdge(EPISEDGE, i, temp));
			/*cout << sActions[i].name << endl;
			curKB.print();
			cout << endl << endl;
			posKB.print();
			cout << endl << endl;
...
void Planner::print(int index, int ident, int depth) {
	...
	if(solEdge[index] == -1) {
		...
		cout << ide << oActions[allEdges[index][solEdge[index]].actionIndex].name << endl;
		print(allEdges[index][solEdge[index]].childNode[0], ident+2, depth+1);
	}
	else {
		cout << ide << sActions[allEdges[index][solEdge[index]].actionIndex].name;
		cout << " +:" << endl;
		print(allEdges[index][solEdge[index]].childNode[0], ident+2, depth+1);
		cout << ide << sActions[allEdges[index][solEdge[index]].actionIndex].name;
		cout << " -:" << endl;
...
vector<char> Planner::checkEpisAlongSol(int index) {
	vector<char> ans(sActions.size(), '0');
	list<int> mq;
	mq.push_back(index);
	while(!mq.empty()) {
...
void Planner::show(ofstream& ros, int index, int indent) {
	allNodes[index].kb.show(ros);
	ros << endl << endl;
	if(solEdge[index] == -1) return ;
	else if(allEdges[index][solEdge[index]].et == ONTICEDGE) {
		for(int j = 0;j < indent;j++) ros << " ";
		ros << "==> " << oActions[allEdges[index][solEdge[index]].actionIndex].name << " :" << endl;
		for(int j = 0;j < indent;j++) ros << " ";
		...
		for(int j = 0;j < indent;j++) ros << " ";
		ros << "==> " << sActions[allEdges[index][solEdge[index]].actionIndex].name << " +:" << endl;
		for(int j = 0;j < indent;j++) ros << " ";
		show(ros, allEdges[index][solEdge[index]].childNode[0], indent+3);
		for(int j = 0;j < indent;j++) ros << " ";
		ros << "==> " << sActions[allEdges[index][solEdge[index]].actionIndex].name << " -:" << endl;
